...
Sentry-бандл активний **у всіх середовищах**, але справжній вимикач — наявність
`SENTRY_DSN`. Порожній DSN повністю вимикає SDK, тож без цієї змінної (як у dev
і test за замовчуванням) нічого не надсилається, хоча конфігурація скрізь
однакова. Це навмисно: prod не є «особливим» середовищем, а різниця між ним і
dev зведена до значення однієї змінної. Кожна подія тегується середовищем
(`environment: %kernel.environment%`), тож dev/test/prod лишаються розрізнюваними
в інтерфейсі Sentry.

Захоплення необроблених винятків виконує вбудований listener бандла; повідомлення
логів рівня `INFO` і вище додаються до подій як breadcrumbs для контексту (у dev
і test вони теж збираються, але без DSN нікуди не йдуть). Трасування
продуктивності вимкнене (`traces_sample_rate: 0.0`), щоб уміститися в безкоштовний
план — за потреби його можна підняти в `config/packages/sentry.yaml`.
