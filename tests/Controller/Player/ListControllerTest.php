    ...
    public static function dataProvider(): iterable
    {
        ...
        yield 'list shows players with towns and current team' => [
            ...
            'afterCallback' => static function (Crawler $crawler, array $objects) {
                ...
                // sorted by lastName ASC: Kotsubynsky, Ukrainka, Franko, Shevchenko
                $names = $rows->each(fn(Crawler $row) => $row->filter('td')->eq(0)->text() |> trim(...));
                static::assertStringContainsString('Коцюбинський', $names[0]);
                static::assertStringContainsString('Українка', $names[1]);
        ...
        yield 'list empty when no players match filter' => [
            'method' => 'GET',
            'uri' => '/players/list?lastName=Неіснуючий',
            'fixtures' => ['Entity/base.yaml', 'Entity/users.yaml'],
            'loginAs' => 'user_with_player',
            'expectedStatus' => 200,
            'afterCallback' => static function (Crawler $crawler, array $objects) {
