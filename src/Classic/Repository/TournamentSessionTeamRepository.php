...
class TournamentSessionTeamRepository extends ServiceEntityRepository
{
    ...
    public function markQuestionRemoved(int $tournamentId, int $questionNumber): void
    {
        $this->getEntityManager()->createQueryBuilder()
            ...
            ->where('a.tournamentSessionTeam IN (
                SELECT st.id FROM App\Classic\Entity\TournamentSessionTeam st
                JOIN st.tournamentSession ts
     ...
     * @param list<int> $sessionTeamIds
     */
    public function deleteByIds(array $sessionTeamIds): void
    {
        if ($sessionTeamIds === []) {
            return;
        }
     ...
     * @return list<int>
     */
    public function findIdsBySession(TournamentSession $session): array
    {
        return $this->createQueryBuilder('st')
            ->select('st.id')
            ->where('st.tournamentSession = :session')
