...
class PlayerRepository extends ServiceEntityRepository
{
    ...
    public function __construct(
        ...
        private PlayerTeamProviderInterface $playerTeamProvider,
    ) {
    ...
    public function findForList(PlayerListRequestDTO $requestDto, ?Season $currentSeason = null): array
    {
        ...
        if ($currentSeason !== null && $rows !== []) {
            $playerIds = array_column($rows, 'id');
            $teams = $this->playerTeamProvider->getTeamsByPlayerIds($playerIds, $currentSeason);

            $rows = array_map(static function (array $row) use ($teams): array {
                $team = $teams[$row['id']] ?? null;
                $row['teamId'] = $team['teamId'] ?? null;
                $row['teamName'] = $team['teamName'] ?? null;
        ...
        return $this->mapper->mapMultiple($rows, PlayerListItemDTO::class);
    }
