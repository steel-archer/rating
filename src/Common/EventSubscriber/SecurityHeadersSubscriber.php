...
final readonly class SecurityHeadersSubscriber implements EventSubscriberInterface
{
    ...
    public function onResponse(ResponseEvent $event): void
    {
        ...
        if ($this->environment === 'prod') {
            ...
            $headers->set('Content-Security-Policy', "default-src 'self'; script-src 'self' 'nonce-$nonce'; style-src 'self'; img-src 'self' data:; font-src 'self'; connect-src 'self'; frame-ancestors 'none'; form-action 'self'; base-uri 'self'");
